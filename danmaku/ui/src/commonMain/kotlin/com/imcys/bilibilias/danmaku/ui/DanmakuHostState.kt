...
class DanmakuHostState(
     ...
     */
    private val floatingSpeedMultiplierState =
        mutableFloatStateOf(danmakuTrackProperties.speedMultiplier)
     ...
     */
    private val fixedDanmakuPresentDuration =
        mutableLongStateOf(danmakuTrackProperties.fixedDanmakuPresentDuration)
     ...
     */
    private suspend fun updateTrackCount(
        count: Int,
        config: DanmakuConfig,
        baseTrackSpeedWidth: Int
    ) {
        uiContext.await()
        ...
        val newFloatingTrackSafeSeparation =
            with(uiContext.density) { danmakuConfig.safeSeparation.toPx() }
    ...
    private suspend fun updateTrackStaticProperties(
        ...
        val newFloatingTrackSafeSeparation =
            with(uiContext.density) { newConfigSafeSeparation?.toPx() }

        floatingTrack.forEach {
            if (newFloatingTrackSafeSeparation != null) it.safeSeparation =
                newFloatingTrackSafeSeparation
    ...
    private suspend fun repopulatePresentDanmaku(currentElapsedFrameTimeNanos: Long) {
        ...
        val presentFixedDanmakuCopied =
            DanmakuCollectionIterator(listOf(topTrack, bottomTrack).flatten()).toList()
    ...
    internal suspend fun interpolateFrameLoop() {
        uiContext.await()
        coroutineScope {
            var currentFloatingTrackSpeed =
                with(uiContext.density) { danmakuConfig.speed.dp.toPx() }
        ...
        withContext(Dispatchers.Main.immediate) {
            ...
            if (danmaku.danmaku.location == DanmakuLocation.NORMAL) {
                ...
                floatingTrack.forEach { track ->
                    ...
                    val right =
                        lastDanmaku.distanceX - (lastDanmaku.danmaku.danmakuWidth + safeSeparation)
                    if (right >= 0) {
                ...
                val placeTimeNanos =
                    (maxDanmakuRight.absoluteValue / floatingTrackSpeed * 1_000_000_000f).toLong()
