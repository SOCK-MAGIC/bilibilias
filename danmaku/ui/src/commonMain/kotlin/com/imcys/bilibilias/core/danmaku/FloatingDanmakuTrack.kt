...
internal class FloatingDanmakuTrack<T : SizeSpecifiedDanmaku>(
    ...
    override fun tryPlace(danmaku: T, placeFrameTimeNanos: Long): FloatingDanmaku<T>? {
        val (upcomingDanmaku, insertionIndex) = checkPlaceableImpl(danmaku, placeFrameTimeNanos)
            ?: return null
        if (insertionIndex < 0) danmakuList.add(upcomingDanmaku) else danmakuList.add(
            insertionIndex,
            upcomingDanmaku
        )
    ...
    private fun checkPlaceableImpl(
        ...
        if (danmakuList.isEmpty()) return if (upcomingDanmaku.isGone()) null else Pair(
            upcomingDanmaku,
            0
        )
        // 如果弹幕右侧超过了轨道左侧, 则不放置
